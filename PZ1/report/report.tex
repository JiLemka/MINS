...
\begin{titlepage}
    ...
    \begin{flushright}
        \textbf{Выполнил:} \\
        Студент группы \rule{2.5cm}{0.4pt} \\
        И.О. Фамилия \\
        \vspace{0.5cm}
        \textbf{Проверил:} \\
        Преподаватель \\
...
\subsection{Контур инференса}

Обработка запроса \texttt{POST /predict} проходит следующие этапы:

\begin{enumerate}
    \item \textbf{Структурная валидация} (Pydantic, схема \texttt{FarmRequest}) --- проверка типов, обязательности полей и диапазонов значений. При ошибке --- HTTP 422, функция инференса не вызывается.
    ...
    \item \textbf{Инференс} --- модель (\texttt{RiskModel.predict()}) возвращает \texttt{risk\_score} $\in [0; 1]$.
...
\hline
Сервисы \newline \texttt{services/} & \texttt{prediction\_service.py} & Сценарий инференса: готовность модели $\to$ бизнес-правила $\to$ модель $\to$ постпроцессинг $\to$ сохранение $\to$ лог \\
...
\subsection{Применение принципов SOLID}

\begin{table}[H]
\centering
\caption{Принципы SOLID в архитектуре сервиса}
\small
\begin{tabular}{|p{3.4cm}|p{12.4cm}|}
\hline
\textbf{Принцип} & \textbf{Реализация} \\
\hline
S --- единственная ответственность & Каждый класс решает одну задачу: модель только считает score, классификатор только переводит score в категорию, правило только проверяет регион, хранилище только хранит. Запись (\texttt{PredictionService}) и чтение (\texttt{PredictionQueryService}) --- разные сервисы. Обработчики маршрутов не содержат бизнес-логики \\
\hline
O --- открытость/закрытость & Новый фактор риска --- новый объект \texttt{RiskFactor} в списке, новое бизнес-правило --- новый класс с методом \texttt{check()}; существующие классы при этом не меняются. Пороги категорий и тексты рекомендаций передаются через конструктор \\
\hline
L --- подстановка Лисков & Любая реализация \texttt{RiskModel} обязана возвращать score в $[0; 1]$; \texttt{RuleBasedRiskModel} гарантирует это ограничением диапазона. Поэтому модель на правилах можно заменить обученной ML-моделью или тестовой заглушкой без изменений в сервисе (это проверяется в тестах) \\
\hline
I --- разделение интерфейсов & Хранилище описано двумя узкими интерфейсами: \texttt{PredictionWriter} (нужен сервису инференса) и \texttt{PredictionReader} (нужен сервису чтения). Каждый сервис знает только нужные ему методы \\
\hline
D --- инверсия зависимостей & Сервисы получают зависимости через конструктор и зависят от протоколов, а не от конкретных классов. Домен не зависит от FastAPI: ошибки не содержат HTTP-кодов, их сопоставляет \texttt{error\_handlers.py}. Реализации собираются в \texttt{container.py}; для тестов приложение создаётся через \texttt{create\_app(Settings(...))} \\
\hline
\end{tabular}
\end{table}

Благодаря такому устройству модель можно заменить обученной scikit-learn-моделью, а хранилище --- SQLite или PostgreSQL. Для этого достаточно написать новый класс, реализующий соответствующий протокол, и подключить его в \texttt{container.py}; сервисы, API-слой и контракт для клиентов при этом не меняются.
...
Для наблюдаемости реализованы: логирование каждого запроса на инференс и его результата (с версией модели), заголовок \texttt{X-Process-Time} с временем обработки каждого запроса. Персональные данные клиента в логи не пишутся --- только идентификаторы и результат.
...
\hline
\textbf{Риск} & \textbf{Проявление} & \textbf{Мера (реализована / планируется)} \\
\hline
...
Потеря результатов & данные исчезают при рестарте & переход на БД (планируется) \\
Смена версии модели & непредсказуемые ответы & \texttt{model\_version} в каждом ответе (реализовано) \\
Долгий инференс & таймауты у клиента & асинхронная обработка, очередь (планируется) \\
Перегрузка & рост задержек & мониторинг \texttt{X-Process-Time}, масштабирование (частично) \\
\hline
\end{tabular}
\end{table}
...
\hline
\multicolumn{5}{|l|}{\textit{Дополнительные кейсы}} \\
\hline
11 & GET /predictions?risk\_level=extreme & 422 & 422 & пройден \\
12 & POST /predict без поля \texttt{debt} & 422 & 422 & пройден \\
13 & POST /predict, модель не готова (\texttt{model\_ready=False}) & 503 & 503 & пройден \\
\hline
\end{tabular}
\end{table}

Для корректного запроса из методички (просрочка 45 дней, 1 дефолт, долг 6,5 млн) получен \texttt{risk\_score = 0.9}, \texttt{risk\_level = high}, рекомендация «Высокий риск. Требуется ручное рассмотрение», что совпадает с ручным расчётом: $0.1 + 0.3 + 0.3 + 0.2 = 0.9$.
...
\begin{itemize}
    ...
    \item Инференс --- самая дорогая операция; отсекать ошибки нужно как можно раньше (принцип fail fast).
...
\subsection{Какой статус корректнее для инференса: 200 OK или 201 Created?}
